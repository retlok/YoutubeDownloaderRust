...
async fn main() {
    ...
    for video in playlist.videos {
        test.push(Box::pin(download_bs(Video::new(video.url).unwrap())));
    }

   let _ =  future::try_join_all(test.into_iter().map(tokio::spawn)).await;
}

async fn download_bs(video: Video) -> Result<(), VideoError> {
    let stream = video.stream().await.unwrap();
    let mut file = File::create(
        "C:\\Users\\main\\Desktop\\Desktop\\Personal_Projects\\YoutubeDownloaderRust\\downloads\\"
            .to_string()
            + &video.get_info().await.unwrap().video_details.title
            + ".mp3",
    )
